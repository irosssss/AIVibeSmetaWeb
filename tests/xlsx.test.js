...
describe("FF&E-детали позиции: артикул/материал/габариты/срок в выгрузке и обратном импорте", () => {
  const rooms = [{ name: "Гостиная", items: [
    { title: "Диван «Морти»", cat: "Мебель", price: 164900, qty: 1, sku: "MT-2200", material: "велюр, олива",
      dims: { w: 220, d: 95, h: 78 }, leadWeeks: 8 },
    { title: "Стол обеденный", cat: "Мебель", price: 60000, qty: 1, material: "дуб" },   // только материал
  ] }];

  it("рабочий режим: все четыре поля переживают round-trip (габариты → {w,d,h})", async () => {
    const c = calc(rooms, 30);
    const res = await roundTrip({ project: "FFE", rooms, grand: c.grand, clientTotal: c.clientTotal,
      markupPct: 30, budget: 500000, mode: "work" });
    expect(byTitle(res, "Диван «Морти»")).toMatchObject({ sku: "MT-2200", material: "велюр, олива", leadWeeks: 8 });
  ...
  it("клиентский режим: артикул и срок — внутренняя кухня, в файл не утекают; материал/габариты остаются", async () => {
    const c = calc(rooms, 30);
  ...
  it("закупочный лист: артикул/материал/габариты доходят до поставщика", async () => {
