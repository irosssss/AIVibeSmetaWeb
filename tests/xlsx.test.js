...
/* bulk-импорт библиотеки товаров (роадмап п.19, восьмой Programa-заход) — строим
   лист напрямую (как importAoa выше), проверяем LedgerXLSX.importLibrary */
async function importLibraryAoa(aoa, sheetName) {
  const wb = XLSXReal.utils.book_new();
  XLSXReal.utils.book_append_sheet(wb, XLSXReal.utils.aoa_to_sheet(aoa), sheetName || "Прайс");
  const buf = XLSXReal.write(wb, { type: "array", bookType: "xlsx" });
  return X.importLibrary({ name: "прайс.xlsx", _buf: buf });
}

describe("Bulk-импорт библиотеки товаров: прайс-лист поставщика (п.19)", () => {
  it("гибкое сопоставление колонок: Наименование/Раздел/Цена/Поставщик/Артикул/Ссылка/Габариты", async () => {
    const res = await importLibraryAoa([
      ["Наименование", "Раздел", "Цена, руб", "Поставщик", "Артикул", "Ссылка", "Габариты, см"],
      ["Диван «Морти»", "Мягкая мебель", "164 900", "Фабрика Линея", "MT-2200", "https://linea.ru/mt2200", "220×95×78"],
      ["Кресло лаунж", "Мягкая мебель", 58000, "Фабрика Линея", "AR-118", "", ""],
    ]);
    expect(res.skipped).toBe(0);
    expect(res.products).toHaveLength(2);
    const sofa = res.products.find((p) => p.title === "Диван «Морти»");
    expect(sofa).toMatchObject({ cat: "Мягкая мебель", price: 164900, unit: "шт", sup: "Фабрика Линея", article: "MT-2200", url: "https://linea.ru/mt2200" });
    expect(sofa.dims).toEqual({ w: 220, d: 95, h: 78 });
    expect(res.products.find((p) => p.title === "Кресло лаунж")).toMatchObject({ price: 58000, sup: "Фабрика Линея", article: "AR-118" });
  });

  it("строки без названия и «Итого» пропускаются и считаются в skipped", async () => {
    const res = await importLibraryAoa([
      ["Наименование", "Цена"],
      ["Стол обеденный", 92000],
      ["", 15000],              // мусорная строка без названия — считается, не импортируется
      ["Итого", 107000],        // итоговая строка — пропускается, НЕ считается в skipped (это не мусор, а ожидаемый хвост)
    ]);
    expect(res.products).toHaveLength(1);
    expect(res.products[0].title).toBe("Стол обеденный");
    expect(res.skipped).toBe(1);
  });

  it("неизвестная единица измерения отбрасывается (тот же словарь FFE_UNITS, что importRoomSpec)", async () => {
    const res = await importLibraryAoa([
      ["Наименование", "Цена", "Ед."],
      ["Ковролin", 1000, "рулон"],
      ["Плитка", 1200, "м²"],
    ]);
    expect(res.products.find((p) => p.title === "Ковролin").unit).toBe("шт");   // мусор → дефолт
    expect(res.products.find((p) => p.title === "Плитка").unit).toBe("м²");
  });

  it("файл без узнаваемой колонки «Наименование» — пустой результат, не падает", async () => {
    const res = await importLibraryAoa([
      ["Дата", "Модель", "Стоимость"],
      ["01.07", "X-100", 5000],
    ]);
    expect(res.products).toEqual([]);
  });

  it("маркер-заголовок находится на втором листе, если на первом его нет", async () => {
    const wb = XLSXReal.utils.book_new();
    XLSXReal.utils.book_append_sheet(wb, XLSXReal.utils.aoa_to_sheet([["Прайс-лист от 01.07.2026"], ["по запросу"]]), "Обложка");
    XLSXReal.utils.book_append_sheet(wb, XLSXReal.utils.aoa_to_sheet([["Наименование", "Цена"], ["Люстра подвесная", 38000]]), "Товары");
    const buf = XLSXReal.write(wb, { type: "array", bookType: "xlsx" });
    const res = await X.importLibrary({ name: "прайс.xlsx", _buf: buf });
    expect(res.products).toHaveLength(1);
    expect(res.products[0]).toMatchObject({ title: "Люстра подвесная", price: 38000 });
  });
});
