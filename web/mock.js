...
(function () {
  ...
  const db = {
    ...
    // сохранённые проекты текущего пользователя — у нового пользователя ПУСТО
    // (М3 аудита запуска 16.07: чужая витрина без пометки «демо» выдавала себя
    // за данные пользователя); демо-проекты доезжают только явной кнопкой —
    // projects.seedDemo() ниже, DEMO_PROJECTS сразу после db
    projects: [],
  ...
  /* Демо-проекты (сид по кнопке, М3): стадии петли Сбор → Согласование → Закупка →
     Сдача (+ Архив), сметы-примеры к ним лежат в project-data.js по этим id.
     demo:true — карточка получает пометку «демо», кнопка «Убрать демо» чистит по флагу. */
  const DEMO_PROJECTS = [
    { id: "p_1", name: "Гостиная на Патриках", room: "Гостиная", style: "Neo Deco", area: 38, items: 12, budget: 480000, updated: "2026-05-27", cover: "living", status: "Сбор", demo: true },
    { id: "p_2", name: "Спальня — тёплый минимализм", room: "Спальня", style: "Тёплый минимализм", area: 18, items: 8, budget: 210000, updated: "2026-05-19", cover: "bedroom", status: "Сдача", demo: true },
    { id: "p_3", name: "Кухня-столовая", room: "Кухня", style: "Сканди", area: 22, items: 12, budget: 365000, updated: "2026-05-11", cover: "kitchen", status: "Согласование", demo: true },
    { id: "p_4", name: "Домашний кабинет", room: "Кабинет", style: "Индустриальный", area: 12, items: 6, budget: 145000, updated: "2026-04-30", cover: "office", status: "Архив", demo: true },
    { id: "p_kirova", name: "Кирова 17к1", room: "3 комнаты · 87,59 м²", style: "По дизайн-проекту", area: 87.59, items: 50, budget: 2700000, updated: "2025-01-30", cover: "living", status: "Закупка", demo: true },
  ];
  ...
  window.LedgerAPI = {
    ...
    projects: {
      ...
      // демо-проекты по явной кнопке (М3): докидываем только отсутствующие,
      // свои проекты пользователя не трогаем. → на бэкенде не появится: демо
      // останется клиентским (или превратится в шаблоны)
      seedDemo: async () => {
        await delay(220);
        const have = new Set(db.projects.map((p) => p.id));
        const add = DEMO_PROJECTS.filter((p) => !have.has(p.id));
        if (add.length) { db.projects = [...db.projects, ...clone(add)]; LS.set("projects", db.projects); }
        return clone(db.projects);
      },
      // снять демо одним действием: удаляем только строки с флагом demo
      clearDemo: async () => {
        await delay(160);
        db.projects = db.projects.filter((p) => !p.demo);
